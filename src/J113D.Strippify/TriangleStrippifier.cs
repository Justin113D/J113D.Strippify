...
namespace J113D.Strippify
{
	...
	public class TriangleStrippifier
	{
		...
		public int[][] Strippify<T>(T[] triangleList, out DistinctMap<T> distinctMap) where T : IEquatable<T>
		{
			...
			if(!DistinctMap.TryCreateDistinctMap(triangleList, out distinctMap))
			{
		...
		public int[][] Strippify(int[] triangleList)
		{
			...
			void AddZTriangle(Triangle tri)
			{
				...
				strips.Add([verts[0].Index, verts[1].Index, verts[2].Index]);
				written++;
				tri.Used = true;
			}
			...
			while(written != triCount)
			{
				...
				if(secNewTri == null)
				{
					...
					strips.Add([prevVert.Index, currentVert.Index, nextVert.Index, thirdVertex]);
					written += 2;
		...
		{
			return [.. JoinedStripEnumerator(strips, reversed)];
		}
	}
}
